...
- `frontend/game.js` owns one local game session. Canvas renders the playfield and `requestAnimationFrame` advances simulation. `frontend/targeting.js` supplies the range and line-of-sight rules for both towers and projectiles. No game update makes a network request.
